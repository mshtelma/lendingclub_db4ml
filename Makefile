...
lint: ## Run the lint and checks
	...
	@make helper-line
	$(PYTHON) -m prospector  --with-tool mypy --profile prospector.yaml
	@make check-formatting

check-formatting: ## Run black checks
	@echo ""
	@echo "${YELLOW}Check code with black:${NORMAL}"
	@make helper-line
	$(PYTHON) -m black --check ./$(PROJECT_NAME)
	$(PYTHON) -m black --check ./tests

format-code: ## fix the code with black formatter.
	@echo ""
...
test: ## Run the tests. (option): file=tests/path/to/file.py
	...
	@make helper-line
	$(PYTHON) -m pytest -vv --cov $(PROJECT_NAME) tests/unit

test-with-html-report: ## Run all tests with html reporter.
	@echo ""
	@echo "${YELLOW}Testing with html report:${NORMAL}"
	@make helper-line
	$(PYTHON) -m pytest tests/unit --cov $(PROJECT_NAME) --cov-report html -s
